...
class RobotView:
    ...
    def setup_ui(self):
        ...
        for i in range(7):
            ...
            tk.Label(frame, text=f"Joint {i+1}:", width=10, font=("Arial", 11, "bold"), anchor="w").pack(side="left")
            entry_val = tk.Entry(frame, width=15, font=("Courier", 12))
            entry_val.insert(0, "0.00") # Auf 2 Nullen gekürzt
    ...
    def bind_controller(self, controller):
        ...
        # Binde Modus-Wechsel und Treeview-Auswahl an den Controller
    ...
    def write_log(self, full_msg, level):
        self.log_area.configure(state='normal')
        self.log_area.insert(tk.END, full_msg, level)
    ...
    def update_joint_entries(self, current_rad, force=False):
        # Dynamisch Frames ein- und ausblenden je nach DOF-Anzahl des Roboters
        for i, frame in enumerate(self.joint_frames):
            if i < len(current_rad):
                ...
                if frame.winfo_ismapped():
                    frame.pack_forget() # Verstecke nicht benötigte Gelenke (z.B. Joint 7 bei 6-DOF)
    ...
    def toggle_entry_states(self, state):
        # Nur sichtbare Entries umschalten
    ...
    def get_entry_poses(self):
        try:
            # Nur Werte aus den aktuell sichtbaren Entries ziehen!
            visible_entries = [e for e, f in zip(self.joint_entries, self.joint_frames) if f.winfo_ismapped()]
    ...
    def update_speed_entry(self, speed):
        """Aktualisiert das Geschwindigkeits-Eingabefeld mit einem neuen Wert."""
...
class ConnectionDialog(tk.Toplevel):
    ...
    def __init__(self, parent, default_ip="", default_user="", default_pass=""):
        ...
        # StringVars für Echtzeit-Überwachung der Eingabefelder
        self.ip_var = tk.StringVar(value=default_ip)
        self.user_var = tk.StringVar(value=default_user)
        self.pass_var = tk.StringVar(value=default_pass)

        # Traces hinzufügen, um bei jedem Tastendruck die Validierung auszuführen
        ...
        # Initiale Validierung (falls Defaults geladen wurden)
